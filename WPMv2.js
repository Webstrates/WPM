...
((window) => {
    ...
    class WPMv2 {
        ...
        static lookupRepoAlias(alias) {
            ...
            //Insert check for actual url alias lookup here
            const aliasMappings = JSON.parse(localStorage.getItem("WPM.repoAliases"));
            if(aliasMappings?.hasOwnProperty(alias)) {
                result = aliasMappings[alias];
            } else {
                ...
                if(alias.startsWith("http") || alias.startsWith("/")) {
                    ...
                    //We had no alias, and the alias did not look like an url, try to make it look like old webstrate url: /alias/?raw
                    result = "/"+alias+"/?raw";
                }
            }

            console.log("Looking up repository alias:", alias, "->", result);
        ...
        static setRepoAliases(aliases) {
            console.log("Setting repo aliases to:", aliases);
            localStorage.setItem("WPM.repoAliases", JSON.stringify(aliases));
        }
    }
    ...
    window.WPMv2 = {
        ...
        setRepoAliases: WPMv2.setRepoAliases,
        version: 2.25,
        revision: "$Id: WPMv2.js 886 2022-04-26 04:58:14Z au182811@uni.au.dk $"
    };
